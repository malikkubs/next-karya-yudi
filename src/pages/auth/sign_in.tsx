...
function PageSignIn() {
  ...
  async function ListTag() {
    await APINew.get("/bg-app/show-all-tag")
      .then((res) => {
        setdata(res);
      })
      .catch((err) => {
  ...
  const mapSubcategories = (items: Item[]): JSX.Element[] => {
    return items?.map((item) => (
      <div key={item.id}>
        ...
        {item.sub && item.sub.length > 0 && (
          <div className="ml-8">{mapSubcategories(item.sub)}</div>
        )}
      </div>
    ));
  };
